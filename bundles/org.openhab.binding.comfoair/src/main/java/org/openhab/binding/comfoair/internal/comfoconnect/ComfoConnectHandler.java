...
import org.openhab.binding.comfoair.internal.comfoconnect.component.ChannelManager;
import org.openhab.binding.comfoair.internal.comfoconnect.component.ConnectionManager;
...
public class ComfoConnectHandler extends BaseThingHandler {
    ...
    public void initialize() {
        ...
        ComfoConnectConnector connector = new ComfoConnectConnector(hostname,
                ComfoAirBindingConstants.COMFOCONNECT_DEFAULT_PORT, clientUuid(config), gatewayUuid(config, hostname));
        this.connector = connector;

        @Nullable
        Integer pinCode = pinCode(config);
        ...
        this.connectionManager = new ConnectionManager(this::connect,
                () -> updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.COMMUNICATION_ERROR,
                        "Gateway connection lost: Keep-alive timeout"),
                () -> updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.COMMUNICATION_ERROR,
                        "Gateway connection lost: Communication error"),
                handler);
        this.channelManager = new ChannelManager(handler, getThing(), this::isLinked, bypassStateWorker,
                this::isConnected, this::updateState);
        this.sensorHandler = handler.getSensorManager();

        scheduler.submit(this::connect);
    }

    private @Nullable Integer pinCode(ComfoConnectConfiguration config) {
        int pinCode;
    ...
    private @NonNull UUID gatewayUuid(ComfoConnectConfiguration config, String hostname) {
        UUID gatewayUuid;
    ...
    private @NonNull UUID clientUuid(ComfoConnectConfiguration config) {
        UUID clientUuid;
    ...
    private void connect() {
        ...
        try {
            ...
            if (sensorHandler != null) {
                protocolHandler.setSensorHandler(sensorHandler);
            }
            ...
            if (this.messageQueue != null) {
                this.messageQueue.startMessageConsumer();
            }
            ...
            if (this.channelManager != null) {
                this.channelManager.subscribeToLinkedChannels();
            }

            updateStatus(ThingStatus.ONLINE);

            if (this.connectionManager != null) {
                this.connectionManager.cancelReconnectAttempt();
            }
            ...
            if (this.connectionManager != null) {
                this.connectionManager.scheduleReconnectAttempt();
            }
    ...
    public void dispose() {
        ...
        if (protocolHandler != null) {
            protocolHandler.setSensorHandler(null);
        }
        ...
        if (this.channelManager != null) {
            this.channelManager.clearSubscriptions();
        }

        // Clear subscribed sensors in sensor manager
        if (this.sensorHandler != null) {
            this.sensorHandler.clearSubscriptions();
        }
    ...
    public void channelLinked(ChannelUID channelUID) {
        if (this.channelManager != null) {
            this.channelManager.channelLinked(channelUID);
        }
    }
    ...
    public void channelUnlinked(ChannelUID channelUID) {
        if (this.channelManager != null) {
            this.channelManager.channelUnlinked(channelUID);
        }
    }
}
