...
/**
 * Sensor implementation for string values with enum transformations.
 * Handles extraction from protobuf messages and maps numeric values to string representations.
 *
 * @author Sascha Knoop - Initial contribution
 */
@NonNullByDefault
public class StringSensor extends Sensor {
    private final Logger logger = LoggerFactory.getLogger(StringSensor.class);
    private Function<Number, String> transformation = (n) -> n.toString();

    public StringSensor(String name, int id, SensorValueType type, String channelId) {
        super(name, id, type, channelId);
    }

    public StringSensor withTransformation(Function<Number, String> transformation) {
    ...
    public @Nullable State valueAsState(Zehnder.CnRpdoNotification message) {
        ...
        Number rawValue = extractValueFrom(payload);
        String transformedValue = transformation.apply(rawValue);

        logger.debug("Sensor {}: raw={}, transformed={}, payload_hex={}", name, rawValue, transformedValue,
                bytesToHex(payload));

        return new StringType(transformedValue);
    }
    ...
    private Number extractValueFrom(byte[] payload) {
        return switch (type) {
     ...
     */
    private int extractUnsignedByte(byte[] payload) {
        if (payload.length >= 1) {
     ...
     */
    private int extractUnsignedShort(byte[] payload) {
        if (payload.length >= 2) {
     ...
     */
    private int extractSignedByte(byte[] payload) {
        if (payload.length >= 1) {
            return payload[0];
        }
        return 0;
